...
func (a *AccountController) PostAdd(c *gin.Context) {
	...
	if len(addRequest.Name) < 1 {
		...
		themisView.AccountView{a.BaseView}.PostAdd(c, addResult)
		return
	}
	...
	accountModule := module.NewAccountModule(a.DB)
...
func (a *AccountController) GetSearch(c *gin.Context) {
	...
	if !isError {
		themisView.AccountView{a.BaseView}.GetSearch(c, http.StatusOK, &result)
	} else {
		themisView.AccountView{a.BaseView}.GetSearch(c, http.StatusBadRequest, nil)
	}
}

func (a *AccountController) PostUpdate(c *gin.Context) {
	result := models.AccountChangeResultJson{}

	loginModule := module.NewLoginModule(a.DB)
	...
	if isErr {
		...
		themisView.AccountView{a.BaseView}.PostUpdate(c, http.StatusBadRequest, &result)
		return
	}
	...
	result.Success = true
	themisView.AccountView{a.BaseView}.PostUpdate(c, http.StatusOK, &result)
}

func (a *AccountController) PostUpdateIcon(c *gin.Context) {
	result := models.AccountUpdateIconRequest{}
	iconFile, err := c.FormFile("icon")
	if err != nil {
		result.Message = "icon file is not found"
		themisView.AccountView{a.BaseView}.PostUpdateIcon(c, http.StatusBadRequest, &result)
		return
	}
	...
	if isErr {
		...
		themisView.AccountView{a.BaseView}.PostUpdateIcon(c, http.StatusInternalServerError, &result)
		return
	}
Retry:
	...
	themisView.AccountView{a.BaseView}.PostUpdateIcon(c, http.StatusOK, &result)
}
...
func (a *AccountController) GetList(c *gin.Context) {
...
func (a *AccountController) GetIcon(c *gin.Context) {
